...
class KnowledgeableStoriesModel(Model):
    ...
    def _calculate_logits_and_softmax(self, encoded_one_flat, encoded_two_flat, i, level):
        ...
        target_mask = torch.diag(torch.ones(logits.size(0)).to(encoded_one_flat.device), i).byte()
        ...
        if level == "sentence":
            mask *= (1 - torch.diag(torch.ones(logits.size(0)).to(encoded_one_flat.device), -i)).byte()
